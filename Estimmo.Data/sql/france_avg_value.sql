...
SELECT 0 AS type, AVG(CAST(ps.value AS decimal) / ps.building_surface_area) AS value
FROM property_sale ps
UNION
SELECT ps.type, AVG(CAST(ps.value AS decimal) / ps.building_surface_area) AS value
FROM property_sale ps
GROUP BY ps.type;
