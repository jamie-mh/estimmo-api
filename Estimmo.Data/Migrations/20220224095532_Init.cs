    ...
    public partial class Init : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            migrationBuilder.Sql(@"
                ...
                SELECT type, id, name, UNACCENT(REPLACE(REGEXP_REPLACE(LOWER(name), '[(),]+', '', 'g'), '-', ' ')) AS search_name, post_code, parent_type, parent_id, geometry FROM
                (
                    ...
                    INNER JOIN region r on d.region_id = r.id
                    UNION
                    SELECT 3 AS type, t.id, CONCAT(t.name, ' (', t.post_code, ')') AS name, t.post_code, 2 AS parent_type, d.id AS parent_id, t.geometry FROM town t
                    INNER JOIN department d on t.department_id = d.id
                    UNION
                    SELECT 4 AS type, a.id, CONCAT(a.number, a.suffix, ' ', s.name, ', ', a.post_code, ' ', t.name) AS name, a.post_code, 3 AS parent_type, s.town_id AS parent_id, a.coordinates AS geometry FROM address a
